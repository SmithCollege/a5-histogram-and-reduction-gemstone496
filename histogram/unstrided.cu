...
#define SIZE 100
#define BLOCKS 1    //   1   4  16   16  128  256 1024 4096
#define THREADS 100 // 100 256 256 1024    .    .    .    .
#define RUNS 100
...
__global__ void histogram(unsigned char *buffer,
			  long size,
			  unsigned int *histo){
  long assignment = (size + blockDim.x*gridDim.x - 1) / (blockDim.x*gridDim.x);
  long init = (threadIdx.x + blockIdx.x*blockDim.x) * assignment;
  long end = init + assignment;
  
  for ( long i = init; i < end && i < size; i++ ) {
...
int main() {
  std::cout << "\nunstrided" << SIZE; // record the size of the run for data collection
